...
func AddSecret(baseDir, env, secretName string) error {
	path := filepath.Join(baseDir, env, "secrets", fmt.Sprintf("%s.asc", secretName))
	_, err := os.Stat(path)
	if err == nil {
		overwrite := false
...
func getAllSecretsPrivate(armoredKey, baseDir, env string) ([]*Secret, error) {
	if os.Getenv("OPENPAAS_PASSPHRASE") == "" {
		defer func() {
			_ = os.Setenv("OPENPAAS_PASSPHRASE", "")
		}()
	}
	pubKeyDir := filepath.Join(baseDir, env, "secrets")
	allSecrets := []*Secret{}
...
func refreshPrivate(armoredKey, baseDir, env string) error {
	...
	for _, secret := range secrets {
		path := filepath.Join(baseDir, env, "secrets", fmt.Sprintf("%s.asc", secret.Name))
		err = os.Remove(path)
		if err != nil {
			return err
		}
...
func initSecretsPrivate(homeDir, baseDir, env string, initFn func() (*keySettings, error)) (bool, error) {
	...
	if e != nil {
		ascKey := ""
		targetDir := filepath.Join(homeDir, ".openpaas")
...
func GetPrivateKey(homeDir string) (*crypto.Key, string, error) {
	...
	targetFile := filepath.Join(targetDir, "private-key.asc")
	if os.Getenv("OPENPAAS_PRIVATE_KEY") != "" {
		ascKey = os.Getenv("OPENPAAS_PRIVATE_KEY")
	} else {
...
func getPublicKeys(baseDir, env string) ([]*crypto.Key, error) {
	pubKeyDir := filepath.Join(baseDir, env, "pubkeys")
	allKeys := []*crypto.Key{}
...
func getPassphrase() (string, error) {
	if os.Getenv("OPENPAAS_PASSPHRASE") != "" {
		return os.Getenv("OPENPAAS_PASSPHRASE"), nil
	}
	secretValue := ""
	...
	err = os.Setenv("OPENPAAS_PASSPHRASE", secretValue)
	if err != nil {
		return "", err
	}
	...
	var qs = []*survey.Question{
		...
		},
		{
			...
			Validate: func(val interface{}) error {
				...
				if captured == "" {
					return fmt.Errorf("passphrase must be set. \nIf you do not want to repeatedly enter it, set it as the environment variable OPENPAAS_PASSPHRASE")
				}
				return nil
			},
		},
